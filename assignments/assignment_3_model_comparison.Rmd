...
df <- read_excel("data/assignment_3_dataset.xlsx")
...
df %>%
  summarise(
    pain_min = min(pain, na.rm=TRUE),
    pain_max = max(pain, na.rm=TRUE),
    age_min = min(age, na.rm=TRUE),
    age_max = max(age, na.rm=TRUE),
    stai_min = min(STAI_trait, na.rm=TRUE),
    stai_max = max(STAI_trait, na.rm=TRUE),
    cat_min = min(pain_cat, na.rm=TRUE),
    cat_max = max(pain_cat, na.rm=TRUE),
    mind_min = min(mindfulness, na.rm=TRUE),
    mind_max = max(mindfulness, na.rm=TRUE),
    serum_min = min(cortisol_serum, na.rm=TRUE),
    serum_max = max(cortisol_serum, na.rm=TRUE),
    saliva_min = min(cortisol_saliva, na.rm=TRUE),
    saliva_max = max(cortisol_saliva, na.rm=TRUE)
  )
...
cutoff <- 4 / nrow(df_clean)
which(cooks_d > cutoff)

df_clean[142, ]

df_clean <- df_clean %>%
  mutate(pain = ifelse(pain > 10 | pain < 0, NA, pain))
...
```{r}
resid_complex <- residuals(model_complex)

qqnorm(resid_complex)
qqline(resid_complex, col = "red")

hist(resid_complex, breaks = 30, main = "Residuals", xlab = "Residuals")
```
...
```{r}
cooks_d <- cooks.distance(model_complex_2)
...
```{r}
resid_complex <- residuals(model_complex_2)
...
```{r}
plot(model_complex_2, which = 1)
```

Homoscedasticty assumption (homogeneity of variance)

```{r}
plot(model_complex_2, which = 3)
...
```{r}
model_simple <- lm(pain ~ age + sex, data = df_clean)

summary(model_simple)
glance(model_simple)

tidy(model_simple, conf.int = TRUE)
standardize_parameters(model_simple)
```
...
```{r}
model_complex_final <- lm(
  pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum,
  data = df_clean
)

summary(model_complex_final)
glance(model_complex_final)

tidy(model_complex_final, conf.int = TRUE)
standardize_parameters(model_complex_final)
```

Compare the two models.

```{r}
anova(model_simple, model_complex_final)
AIC(model_simple, model_complex_final)
```
